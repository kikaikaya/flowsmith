...
defaults:                     # 全ステップ共通のデフォルト
  ...
  max_asks: 2                 # 1 ステップあたりの臨時質問（ask）上限。超過 = seized
...
```flowsmith-result
step: write_specs
status: pass          # done | pass | fail | yes | no | again | ask | seized
rounds: 2
outputs:
  - flowsmith-out/specs/
...
|------|------------------|
| make | `done` / `ask` |
| make-check | `pass` / `seized`（maker は `ask` も可） |
| branch | `yes` / `no` / `ask` |
| loop | `done` / `again` / `ask` / `seized` |

ブロックが欠落・不正な場合、エンジンは推測で進まず `seized`（停止・人間待ち）にする。

### `ask` — 臨時質問（社員の「ここ確認させてください」に相当）

指示に致命的な曖昧さがあるとき、agent は推測で進まず `status: ask` + `questions:`（1〜3 件）を返す。
エンジンはその場で人間に質問を提示し、回答をプロンプトに注入して同じステップを再派発する。
**seized と違いフローは止まらない**（回答が得られ次第続行する正常系）。
同一ステップの質問が `max_asks`（既定 2）を超えた場合は seized（brief 自体の欠陥とみなし人間が直す）。
